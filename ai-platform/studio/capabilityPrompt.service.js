...
export async function getCapabilityPrompt(capabilityKey) {
  ...
  let fallback = null;
  try { fallback = typeof cap.fallbackPrompt === "function" ? cap.fallbackPrompt({}) : null; } catch { fallback = null; }
  return {
    capabilityKey, name: cap.name, promptKey,
    fallback: typeof fallback === "string" ? fallback : null, // the hardcoded prompt
...
export async function setCapabilityPrompt({ capabilityKey, body, actorId = null }) {
  const cap = getCapability(capabilityKey);
