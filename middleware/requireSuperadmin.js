...
export default async function requireSuperadmin(req, res, next) {
  ...
  let decoded;
  try {
    decoded = verifySuperadminAccessToken(auth.slice(7));
    if (
      !decoded ||
  ...
  try {
    if (!(await isSuperadminSessionActive(decoded.sub, decoded.sid))) {
      return res.status(401).json({ error: "Super Admin session expired or revoked" });
    }
  } catch (error) {
    console.error("[superadmin auth] session validation failed:", error.message);
